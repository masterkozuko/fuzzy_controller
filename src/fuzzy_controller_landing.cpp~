...
void setEngineLargeTag_main_control(TagInfoAce& ts)
{
	...
	outputVariable3->setDefuzzifier(new fl::Centroid(500));
	...
	outputVariable4->setDefuzzifier(new fl::Centroid(500));
	...
	ruleblock1->addRule(fl::Rule::parse("if orientation_x is way_left then cmd_vel_z_rot is large_turn_right", engine));
	ruleblock1->addRule(fl::Rule::parse("if orientation_x is a_little_left then cmd_vel_z_rot is small_turn_right", engine));
	ruleblock1->addRule(fl::Rule::parse("if orientation_x is On_Target then cmd_vel_z_rot is hold_direction", engine));
	ruleblock1->addRule(fl::Rule::parse("if orientation_x is a_little_right then cmd_vel_z_rot is small_turn_left", engine));
	ruleblock1->addRule(fl::Rule::parse("if orientation_x is way_right then cmd_vel_z_rot is large_turn_left", engine));
	ruleblock1->addRule(fl::Rule::parse("if displacement_x is way_left then cmd_vel_y_linear is large_move_right", engine));
	ruleblock1->addRule(fl::Rule::parse("if displacement_x is too_left then cmd_vel_y_linear is small_move_right", engine));
	ruleblock1->addRule(fl::Rule::parse("if displacement_x is centered then cmd_vel_y_linear is do_nothing", engine));
	ruleblock1->addRule(fl::Rule::parse("if displacement_x is too_right then cmd_vel_y_linear is small_move_left", engine));
	ruleblock1->addRule(fl::Rule::parse("if displacement_x is way_right then cmd_vel_y_linear is large_move_left", engine));
	ruleblock1->addRule(fl::Rule::parse("if displacement_y is way_low then cmd_vel_x_linear is large_move_backward", engine));
	ruleblock1->addRule(fl::Rule::parse("if displacement_y is too_low then cmd_vel_x_linear is small_move_backward", engine));
	ruleblock1->addRule(fl::Rule::parse("if displacement_y is centered then cmd_vel_x_linear is do_nothing", engine));
	ruleblock1->addRule(fl::Rule::parse("if displacement_y is too_high then cmd_vel_x_linear is small_move_foward", engine));
	ruleblock1->addRule(fl::Rule::parse("if displacement_y is way_high then cmd_vel_x_linear is large_move_foward", engine));
	ruleblock1->addRule(fl::Rule::parse("if displacement_z is way_high then cmd_vel_z_linear is large_move_down", engine));
	ruleblock1->addRule(fl::Rule::parse("if displacement_z is too_high then cmd_vel_z_linear is small_move_down", engine));
	ruleblock1->addRule(fl::Rule::parse("if displacement_z is centered then cmd_vel_z_linear is do_nothing", engine));
	ruleblock1->addRule(fl::Rule::parse("if displacement_z is too_low then cmd_vel_z_linear is small_move_up", engine));
